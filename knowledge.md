...
- `components/CustomerMenu.tsx` — guest view: categories, cart, order placement via `?table=<id>`.
- `components/OwnerCabinet.tsx` — owner view: dish/category/table CRUD, order status flow, per-table QR download (`qrcode` pkg).
...
## Roadmap
`PLAN.md` holds the production roadmap, fixed decisions, and status (Phases 0–3 done, Phase 4 deploy in progress; post-release items: analytics, web push, PWA, online payment). `README.md` is the (slightly stale) Ukrainian overview — treat `AGENTS.md` as authoritative.
